...
const MAX_DAYS_PER_CHUNK = 30;  // +30 дней → интервал 31 день включительно
const MAX_IDS_PER_REQUEST = 50;  // лимит API

/**
 * Основная функция для выгрузки и записи данных по частям.
 * Разбивает на пачки по датам (до 31 дня) и по кампаниям (до 50 за раз).
 * Пишет в лист постепенно, чтобы пользователь видел прогресс.
 * 
 * @param {Array<Object>} campaignIdsArray - массив объектов вида [{advertId: 12345, type: 8}, ...] (type обязателен для 'Тип компании')
 * @param {string} begin - "2025-10-01"
 * @param {string} end   - "2025-11-15"
 * @param {string} sheetName - Имя листа, куда писать (по умолчанию 'Реклама')
 */
function fetchAndWriteRaklamaData(campaignIdsArray, begin, end, sheetName = 'Реклама') {
  if (!Array.isArray(campaignIdsArray) || campaignIdsArray.length === 0) {
    throw new Error("campaignIdsArray должен быть непустым массивом объектов с advertId и type");
  }
  ...
  // Если таблица пустая (нет даже заголовков) — пишем заголовки на строку 1
  if (lastRow === 0) {
    const order = [
      "views", "clicks", "ctr", "cpc", "sum", "atbs", "orders", "cr", "shks", "sum_price",
      "name", "nmId", "date", "advertId", "Тип компании", "canceled", "appType"
  ...
  let rowIndex = lastRow + 1;;  // Начинаем запись данных со второй строки
  ...
  if (currentDate > finishDate) {
    throw new Error("begin позже end — проверь даты");
  }

  // Внешний цикл: по пачкам дат
  while (currentDate <= finishDate) {
    let chunkEndDate = new Date(currentDate);
    chunkEndDate.setDate(chunkEndDate.getDate() + MAX_DAYS_PER_CHUNK);

    if (chunkEndDate > finishDate) {
      chunkEndDate = new Date(finishDate);
    }

    const beginDateStr = Utilities.formatDate(currentDate, "GMT", "yyyy-MM-dd");
    const endDateStr = Utilities.formatDate(chunkEndDate, "GMT", "yyyy-MM-dd");

    Logger.log(`Внешний цикл: обрабатываем даты ${beginDateStr} — ${endDateStr}`);

    // Внутренний цикл: по пачкам кампаний (по 50)
    let startIdx = 0;

    while (startIdx < campaignIdsArray.length) {
      const endIdx = Math.min(startIdx + MAX_IDS_PER_REQUEST, campaignIdsArray.length);
      const idsSlice = campaignIdsArray.slice(startIdx, endIdx);

      Logger.log(`Внутренний цикл: кампании с ${startIdx} по ${endIdx-1} (кол-во: ${idsSlice.length})`);

      // Получаем сырые данные для этой пачки
      const rawData = getCompanyData(idsSlice, beginDateStr, endDateStr);

      if (rawData && rawData.length > 0) {
        // Подготавливаем данные
        const preparedData = prepareCompanyData(rawData, campaignIdsArray);

        // Пишет в лист и возвращает новый rowIndex
        rowIndex = writeTableToSheet2(preparedData, sheetName, rowIndex);
      } else {
        Logger.log("Нет данных для этой пачки — пропускаем");
      }

      startIdx += MAX_IDS_PER_REQUEST;

      // Пауза, чтобы не банили (API WB чувствителен к частоте)
      Utilities.sleep(22000);  // 22 секунды
    }

    // Переходим к следующей пачке дат
    currentDate = new Date(chunkEndDate);
    currentDate.setDate(currentDate.getDate() + 1);

    // Дополнительная пауза между периодами
    Utilities.sleep(22000);
  }

  Logger.log("Выгрузка завершена. Всего строк в листе: " + (rowIndex - 1));
}

/**
 * Функция для получения сырых данных (аналог getRaklamaData, но с фиксированными лимитами).
 * Принимает ТОЛЬКО до 50 ids и до 31 дня.
 * 
 * @param {Array<Object>} idsSlice - срез кампаний [{advertId: 12345, type: 8}, ...]
 * @param {string} beginDateStr
 * @param {string} endDateStr
 * @return {Array<Object>} сырой JSON от API
 */
function getCompanyData(idsSlice, beginDateStr, endDateStr) {
  const headers = { "Authorization": TOKEN };

  // Собираем строку ids
  const idsString = idsSlice
    .map(item => item.advertId)
    .filter(id => id != null && !isNaN(id))
    .join(",");

  if (!idsString) {
    throw new Error("Пустой idsString");
  }
  ...
  const url = "https://advert-api.wildberries.ru/adv/v3/fullstats?" + queryString;
  ...
  try {
    ...
    if (code !== 200) {
      Logger.log(`Ошибка ${code}: ${text}`);
      throw new Error(`API вернул ${code}: ${text.substring(0, 200)}`);
    }
    ...
    Logger.log("Исключение в getCompanyData: " + e);
    return [];  // Возвращаем пустой, чтобы не ломать цикл
  }
}
 ...
 */
function writeTableToSheet2(preparedData, sheetName, startRowIndex = 2) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName);
  if (!sheet) {
